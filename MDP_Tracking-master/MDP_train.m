...
is_show = 0;   % set is_show to 1 to show tracking results in training 0
is_save = 1;   % set is_save to 1 to save trained tracker
is_text = 0;   % set is_text to 1 to display detailed info in training 0
...
while 1
    ...
    while fr <= seq_num
        ...
        elseif tracker.state == 3
            ...
            if isempty(index_det) == 0      %bu kong
                % compute reward
...
%     filename = sprintf('%s/%s_tracker.mat', opt.results, seq_name);
    filename = sprintf('%s/%s%s_tracker.mat', opt.results, opt.method,seq_name);
