...
class KCFTracker:
    def __init__(self, hog=False, fixed_window=True, multiscale=False):
        ...
        self.padding = 1.753  # extra area surrounding the target  ori 2.5(21.1)   1.5(21.3)  1.75(21.4)  1.85(21.3)  1.65(21.3)  1.8(21.3)  1.753(21.4)
        self.output_sigma_factor = 0.1  # bandwidth of gaussian target  ori 0.125

        if (hog):  # HOG feature
            # VOT
            self.interp_factor = 0.02  # linear interpolation factor for adaptation  ori 0.012
            self.sigma = 0.5  # gaussian kernel bandwidth  ori 0.6
    ...
    def createHanningMats(self):
        # if (self.size_patch[1] == 1):
        #     self.size_patch[1] = 7
        #     print("In createHanningMats changed self.size_patch[1] is %s" % self.size_patch[1])
        # if (self.size_patch[0] == 1):
        #     self.size_patch[0] = 24
        #     print("In createHanningMats changed self.size_patch[0] is %s" % self.size_patch[0])

        hann2t, hann1t = np.ogrid[0:self.size_patch[0], 0:self.size_patch[1]]
        with np.errstate(invalid='ignore'):
            # print("self.size_patch[1]=%s" % self.size_patch[1])
            # print("self.size_patch[0]=%s" % self.size_patch[0])
    ...
    def getFeatures(self, image, inithann, scale_adjust=1.0):
        ...
        if (z.shape[1] != self._tmpl_sz[0] or z.shape[0] != self._tmpl_sz[1]):
            # z = cv2.resize(z, tuple(self._tmpl_sz)) #python2
        ...
        if (self._hogfeatures):
            ...
            #if (self.size_patch[1] == 1):
            #    self.size_patch[1] = 7
            #    print("In get_futures changed self.size_patch[1] is %s" % self.size_patch[1])
            #if (self.size_patch[0] == 1):
            #    self.size_patch[0] = 24
            #    print("In get_futures changed self.size_patch[0] is %s" % self.size_patch[0])
    ...
    def train2(self, x, train_interp_factor):
        k = self.gaussianCorrelation(x, x)
        alphaf = complexDivision(self._prob, fftd(k) + self.lambdar)
