...
def _create_network(incoming, reuse=None, weight_decay=1e-8):
    ...
    # network = slim.max_pool2d(network, [3, 3], [2, 2], padding="SAME", scope="pool1")
    #
    ...
    # Features in rows, normalize axis 1. tf.reduce_sum(tf.square(features), [1], keepdims=True)
...
def main():
    ...
    with tf.Session(graph=tf.Graph()) as session:
        input_var = tf.placeholder(
            tf.uint8, (1, 128, 64, 3), name="images")   #None
        image_var = tf.map_fn(
