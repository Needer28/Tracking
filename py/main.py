...
from bb_proc import get_iou, bb_update_vp2, ds_score, bb_update_vp, bb_pred, bb_pred_kcf
...
# %% from tracker_lstm_cosine_analyses_occulusion_det.py
...
# // length(track)<7
                    ...
                    elif best_match_score > thcos:   # cosine metric learning
                        for det_num, det in enumerate(dets_f):        
                            if matched_flag[det_num] == True:
                                match_cosdistance[det_num] = 100   #inf
                                match_scores[det_num] = 0
                                ...
                                match_cosdistance[det_num] = dis_cos
                                # dis_eu = nn_matching._nn_euclidean_distance([trackfeature], [detfeature])
                                # match_eudistance[det_num] = dis_eu

                        minindex = match_cosdistance.argmin()
                        best_match3 = dets_f[minindex]
                        best_match_score3 = match_cosdistance.min()

                        if best_match_score3 < max_eudistance:
                            bb_update_vp(id_, best_match3[2:6], res)
                            id_['bb'].append(best_match3[2:6])
                            id_['max_score'] = max(id_['max_score'], best_match3[-1])
                            id_['features'].append(detections[minindex].feature)
                            id_['pred'] = 0
            ...
            # creates new ids   ==============================add detection feature
